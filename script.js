    ...
    if (ladrido || movimiento || vibrando) {
      if (Date.now() - ultimaNotif > cooldownNotif) {
        ...
        if (ladrido) descripcion = "Se detectó un ladrido.";
          ...
          descripcion = "El dispositivo activó vibración por alerta.";
...
document
  .getElementById("btnDetenerNotif")
...
document
  .getElementById("volverNotificaciones")
...
//  ESTADO GLOBAL
// -----------------------------
let modoActualSeleccionado = null;
let modoActivo = null;
let inicioModo = null;

let ultimaNotif = 0;
let cooldownNotif = 10000;
...
const tarjetasPersona = document.querySelectorAll(
  ".modo-persona .persona-card"
);
...
document
  .getElementById("volverHomeDesdeActivo")
  .addEventListener("click", () => {
    ocultarTodasLasPantallas();
    document.querySelector(".home").style.display = "block";
    activarIconoHome();
...
document
  .getElementById("botonModoMascota")
  .addEventListener("click", async () => {
...
document
  .getElementById("detenerModoDetalle")
  .addEventListener("click", async () => {
    modoActivo = null;
    inicioModo = null;
...
document
  .getElementById("modoPersonaCard")
  .addEventListener("click", irAModoPersona);
document
  .getElementById("volverModosPersona")
  .addEventListener("click", volverAModos);
